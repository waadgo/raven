...
from taming.data.utils import AsegDatasetWithAugmentation3D_v2, AsegDatasetWithAugmentation3D
import math
import signal
...
class DataModuleFromConfig3D(pl.LightningDataModule):
    ...
    def __init__(self, batch_size, train=None, validation=None, test=None,
                 ...
                 lazy_loading=False,
                 seed=23,
                 # MODIFIED: Accept crop_size from YAML, with a default
                 crop_size=(32, 32, 32)):
        super().__init__()
        ...
        self.crop_size = crop_size  # MODIFIED: Store crop_size
    ...
    def setup(self, stage=None):
        if "validation" in self.dataset_configs:
            ...
            if "params" in config and "dataset_paths_files" in config["params"]:
                ...
                for p in val_paths:
                    print(f"  {p}")
                # MODIFIED: Pass self.crop_size to the dataset
                self.val_dataset = AsegDatasetWithAugmentation3D_v2(
                    val_paths,
                    transforms=None,
        ...
        if "test" in self.dataset_configs:
            ...
            if "params" in config and "dataset_paths_files" in config["params"]:
                ...
                for p in test_paths:
                    print(f"  {p}")
                # MODIFIED: Pass self.crop_size to the dataset
                self.test_dataset = AsegDatasetWithAugmentation3D_v2(
                    test_paths,
                    transforms=None,
                    lazy_loading=self.lazy_loading,
                    crop_size=self.crop_size
        ...
        if "train" in self.dataset_configs:
            ...
            if "params" in config and "dataset_paths_files" in config["params"]:
                ...
                # MODIFIED: Pass self.crop_size to the dataset
                self.train_dataset = AsegDatasetWithAugmentation3D_v2(
                    self.full_train_filelist,
...
if __name__ == "__main__":
    ...
    try:
        ...
        default_callbacks_cfg = {
            "setup_callback": {
                "target": "main_hdf5_3d_v6_3_FiLM.SetupCallback",
                "params": {
                    "resume": opt.resume,
                    "now": now,
                    "logdir": logdir,
            ...
            "image_logger": {
                "target": "main_hdf5_3d_v6_3_FiLM.ImageLogger3D",
                "params": {
                    "batch_frequency": 50,
                    "max_images": 4,
                    "clamp": True
            ...
            "custom_tqdm_bar": {
                "target": "main_hdf5_3d_v6_3_FiLM.CustomTQDMProgressBar",  # adjust this path accordingly
                "params": {
                    "refresh_rate": 1
                }
            },
