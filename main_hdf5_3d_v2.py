...
class CustomRichProgressBar(RichProgressBar):
    def get_metrics(self, trainer, pl_module):
        # Get the base metrics from RichProgressBar.
        ...
        if torch.cuda.is_available():
            ...
            metrics["vram_alloc"] = f"Alloc: {alloc_GB:.2f}GB ({alloc_percent:.1f}%)"
            metrics["vram_resv"] = f"Resv: {resv_GB:.2f}GB ({resv_percent:.1f}%)"

            # Add training metrics from trainer.logged_metrics.
            # (These keys must match what you log with prog_bar=True in your training_step.)
            metrics["t_ae_st"] = trainer.logged_metrics.get("train/aeloss_step", "N/A")
            metrics["t_tot_st"] = trainer.logged_metrics.get("train/total_loss_step", "N/A")
            metrics["t_lvar_st"] = trainer.logged_metrics.get("train/logvar_step", "N/A")
            metrics["t_kl_st"]   = trainer.logged_metrics.get("train/kl_loss_step", "N/A")
            metrics["t_nll_st"]  = trainer.logged_metrics.get("train/nll_loss_step", "N/A")
            metrics["t_rec_st"]  = trainer.logged_metrics.get("train/rec_loss_step", "N/A")
            metrics["t_dw_st"]   = trainer.logged_metrics.get("train/d_weight_step", "N/A")
            metrics["t_dfact_st"] = trainer.logged_metrics.get("train/disc_factor_step", "N/A")
        return metrics
...
#             resv_percent = mem_reserved / total_mem * 100
#             postfix_str = "\n".join([
#                 f"vra: {alloc_GB:.2f}GB ({alloc_percent:.1f}%)",
#                 f"vrr: {resv_GB:.2f}GB ({resv_percent:.1f}%)",
#                 f"t_ae_st: {trainer.logged_metrics.get('train/aeloss_step', 'N/A')}",
#                 f"t_tot_st: {trainer.logged_metrics.get('train/total_loss_step', 'N/A')}",
#                 f"t_lvar_st: {trainer.logged_metrics.get('train/logvar_step', 'N/A')}",
#                 f"t_kl_st: {trainer.logged_metrics.get('train/kl_loss_step', 'N/A')}",
#                 f"t_nll_st: {trainer.logged_metrics.get('train/nll_loss_step', 'N/A')}",
#                 f"t_rec_st: {trainer.logged_metrics.get('train/rec_loss_step', 'N/A')}",
#                 f"t_dw_st: {trainer.logged_metrics.get('train/d_weight_step', 'N/A')}",
#                 f"t_dfact_st: {trainer.logged_metrics.get('train/disc_factor_step', 'N/A')}",
#             ])
...
#             resv_percent = mem_reserved / total_mem * 100
#             postfix_str = "\n".join([
#                 f"vram_alloc: {alloc_GB:.2f}GB ({alloc_percent:.1f}%)",
#                 f"vram_resv: {resv_GB:.2f}GB ({resv_percent:.1f}%)",
#                 f"val/aeloss_step: {trainer.logged_metrics.get('val/aeloss_step', 'N/A')}",
#                 f"val/total_loss_step: {trainer.logged_metrics.get('val/total_loss_step', 'N/A')}",
#                 f"val/logvar_step: {trainer.logged_metrics.get('val/logvar_step', 'N/A')}",
#                 f"val/kl_loss_step: {trainer.logged_metrics.get('val/kl_loss_step', 'N/A')}",
#                 f"val/nll_loss_step: {trainer.logged_metrics.get('val/nll_loss_step', 'N/A')}",
#                 f"val_rec_loss_step: {trainer.logged_metrics.get('val_rec_loss_step', 'N/A')}",
#                 f"val/d_weight_step: {trainer.logged_metrics.get('val/d_weight_step', 'N/A')}",
#                 f"val/disc_factor_step: {trainer.logged_metrics.get('val/disc_factor_step', 'N/A')}",
#             ])
#             pb.set_postfix_str(postfix_str)

#     def init_train_tqdm(self):
#         bar = super().init_train_tqdm()
#         bar.bar_format = "{l_bar}{bar}\n{r_bar}"
#         return bar
...
if __name__ == "__main__":
    ...
    try:
        ...
        default_callbacks_cfg = {
            ...
            "rich_progress_bar": {
                "target": "main_hdf5_3d_v2.RichProgressBar",
                "params": {}
            },
