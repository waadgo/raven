...
class PopulationDataset:
    """
    Class to load all images listed in a CSV file and extract 3D patches (from the whole volume).
    Each patch is filtered according to the rule: if it has <20% foreground, then it is kept only
    50% of the time (randomly).
    """
    ...
    def create_hdf5_dataset(self, patch_size=(64, 64, 64), stride=(32, 32, 32)):
        ...
        Creates HDF5 dataset(s) of 3D patches by:
          ...
          2) Using the entire image (no bounding box cropping).
          3) Symmetrically padding the volume as needed.
          4) Extracting 3D patches.
          5) Randomly discarding 50% of patches with <20% foreground.
          6) Saving the results to an HDF5 file.
          7) Saving a snapshot PNG grid per subject for quick inspection.
        """
        start_d = time.time()
        ...
        # Create snapshots folder structure.
        ...
        for current_split in range(self.total_splits):
                ...
                for row in csv_reader:
                    if is_idx_within_split(idx, current_split, split_indices, self.total_splits):
                        ...
                        try:
                            ...
                            zoom, volume, mask = load_image_masked(current_subject_fpath, im_threshold=self.im_threshold)

                            # Instead of cropping to a bounding box, use the full volume.
                            ...
                            # Save a snapshot PNG grid for this subject if any patches were extracted.
                            if patches.shape[0] > 0:
                                # Randomly sample up to 16 patches for the snapshot.
                                n_samples = min(patches.shape[0], 16)
                                sample_indices = np.random.choice(patches.shape[0], size=n_samples, replace=False)
                                sampled_patches = patches[sample_indices]
                                # For visualization, take the middle slice along the third dimension.
                                patch_slices = [patch[:, :, patch.shape[2] // 2] for patch in sampled_patches]
                                ...
                                snap_fname = os.path.join(snapshot_dir, subject_basename + ".png")
                                plt.savefig(snap_fname)
                                plt.close(fig)
                                print(f"Saved snapshot for {subject_basename} to {snap_fname}")

                                # Append patches to the global array.
                                all_patches = np.concatenate([all_patches, patches], axis=0)
                                n_patches = patches.shape[0]
                                all_zooms_x += [zoom[0]] * n_patches
                                all_zooms_y += [zoom[1]] * n_patches
                                all_zooms_z += [zoom[2]] * n_patches
                                subjects += [subject_basename.encode("ascii", "ignore")] * n_patches
                            else:
                                print(f"No patches extracted for {current_subject_fpath}")

                            end = time.time() - start
                            print(f" -> Extracted {patches.shape[0]} patches in {end:.3f}s")
            ...
            # If the HDF5 file already exists, skip processing this split.
            # if os.path.exists(current_hdf5_fname):
            #     print(f"HDF5 file {current_hdf5_fname} already exists. Skipping split {current_split + 1}.")
            #     continue
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='HDF5 Creation for 3D Patches with Symmetric Padding, Random Filtering, Robust Rescaling, and Snapshot Visualization'
    )
    ...
    parser.add_argument('--hdf5_path', type=str,
                        default="/data/dadmah/gonwal2/Documents/SuperResolution/datasets/RAVEN_LDM_3D_test/",
                        help='Folder to save the HDF5 dataset(s).')
    parser.add_argument('--csv_file_images', type=str,
                        default="/data/dadmah/gonwal2/Documents/datasets/Datasets_raw/ALL_pp1_datasplit_csvs_dh/",
    ...
    parser.add_argument('--patch_size', nargs=3, type=int, default=[96, 96, 96],
                        help="Size of the 3D patch: [px, py, pz].")
    parser.add_argument('--stride', nargs=3, type=int, default=[96, 96, 96],
                        help="Stride of the sliding window: [sx, sy, sz].")
    ...
    if os.path.isdir(args.csv_file_images):
        ...
        for csv_file in csv_files:
            ...
            #     print(f"HDF5 file {hdf5_full_path} already exists. Skipping {full_csv_path}.")
            #     continue
